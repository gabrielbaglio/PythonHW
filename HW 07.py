...
def litCricFriend(wordList, text):
    ...
    for element in editList:
        if element[0] in string.punctuation or element[-1] in string.punctuation:
            newList.append(element.replace(',' or ':' or ';' or '.',''))  
    ...
    for element in wordList:
        newerList.appened(element.lower())
    for element in newList:
        if element in newerList:
            print(wordList.count(element))
